...
if (isset($_POST["pag"]) && $_POST["pag"] == "pwdUpdate2" && isset($_POST["newpwd"]) && isset($_SESSION["user"])) {
    ...
    $data = date("%Y-%m-%d");
    $q = "update " . $tabella . " set " . $campo1 . " = " . password_hash($_POST["newpwd"], PASSWORD_DEFAULT) . ", " . $campo2 . " = " . $data . " where " . $campo3 . " = '" . $_SESSION["user"][$campo3] . "'";
    $ris = mysqli_query($conn, $q) or die("Errore nell'aggiornamento della password");
    $_SESSION["user"][$campo1] = password_hash($_POST["newpwd"], PASSWORD_DEFAULT);
    header("Location: index.php");
}
...
if ($_POST["pag"] == "fotoprofilo2" && isset($_SESSION["user"]) && isset($_SESSION["user-type"])) {
    ...
    if(isset($_FILES["miofile"]) && isset($_FILES["miofile"]["tmp_name"])){
